...
class FileSchedulerTest {
    ...
    @Test
    void claimsEachAcceptedFileOnceAndCompletesLease() throws Exception {
        Path root = Files.createTempDirectory(Path.of("target"), "scheduler-");
        ...
        try (FileQueueEventJournal journal = new FileQueueEventJournal(
                root.resolve("journal"), journalConfiguration(), new BinaryV1JournalCodec())) {
            QueueProjectionService projection = new QueueProjectionService(
                    ...
                    clock,
                    null);
            TestVolume volume = new TestVolume(root.resolve("volume"));
            DefaultStoragePool pool = new DefaultStoragePool(
                    ...
                    volume,
                    clock);
            String first = pool.write(tenant, ContentSources.of(new byte[] {1}), null);
            String second = pool.write(tenant, ContentSources.of(new byte[] {2}), null);

            assertThat(pool.claimBatch(tenant, 10))
                    .extracting(ClaimedFile::location)
                    .extracting(location -> location.fileKey())
                    .containsExactlyInAnyOrder(first, second);
            assertThat(pool.claimNext(tenant)).isEmpty();
            ClaimedFile one = pool.claimBatch(tenant, 2).stream().findFirst().orElse(null);
            assertThat(one).isNull();
        }
    }

    @Test
    void failedLeaseBecomesAvailableAfterConfiguredDelay() throws Exception {
        Path root = Files.createTempDirectory(Path.of("target"), "scheduler-fail-");
        Clock clock = Clock.fixed(Instant.parse("2026-09-19T00:00:00Z"), ZoneOffset.UTC);
        TenantContext tenant = new TenantContext("tenant-a", TenantStatus.ENABLED, clock.instant(), clock.instant());
        SqliteQuotaRepository quota = new SqliteQuotaRepository(
                root.resolve("quota"), SqliteConnectionFactory.defaults(), clock, ignored -> 20);
        ...
        try (FileQueueEventJournal journal = new FileQueueEventJournal(
            ...
            DefaultStoragePool pool = new DefaultStoragePool(
                    ...
                    new io.github.cocosip.stow.config.RetryConfiguration(
                            2, Duration.ofSeconds(5), true, Duration.ofSeconds(5)));
            pool.write(tenant, ContentSources.of(new byte[] {1}), null);
            ClaimedFile claimed = pool.claimNext(tenant).orElseThrow();
            pool.fail(claimed.lease(), "failed");
            pool.fail(claimed.lease(), "duplicate");
            assertThat(pool.claimNext(tenant)).isEmpty();
            assertThat(pool.status(tenant, claimed.location().fileKey())).isEqualTo(FileProcessingStatus.FAILED);
        }
    }

    private static JournalConfiguration journalConfiguration() {
